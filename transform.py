    ...
    with open(archivo, newline="", encoding="utf8") as file:
        ...
        for i, row in enumerate(reader):
            for idx in range((len(row))):
                tempDic[keys[idx]] = validateDatatypeJson(
                    row[idx], schema[idx], i, keys[idx], row
                )
...
def getJson(json_filename: str, csv_data):
    with open(json_filename, "w", encoding="utf-8") as jsonfile:
        for row in csv_data:
            jsonfile.write(json.dumps(row) + "\n")
    jsonfile.close()
    print("JSON " + json_filename + " created!")
...
def splitJson(csv_data, parts: int):
    ...
    print(currentRoot)
    jsondir = ""
    extractPath = os.path.join(currentRoot, "JSON")
    ...
    for i in rlimits:
        if s < len(rlimits):
            ...
            getJson(filename, csv_data[temprange.start : temprange.stop])
            ...
            # json
            filename = jsondir + "inegi" + str(idx_filename) + ".json"
            getJson(filename, csv_data[-diff:])
            compressFile(filename)
            idx_filename = idx_filename + 1

    print("JSON partitioned!")
